...
rule compare_replicate_closeness:
    conda:
        '../envs/bedtools.yml'
    input:
        rep1='output/{sample_1}/depth/{mode}/deep/{region}.{strand}.sorted.bed',
        rep2='output/{sample_2}/depth/{mode}/deep/{region}.{strand}.sorted.bed',
        genome='rawdata/hg19/hg19.chrom.sizes'
    output:
        'output/compare_replicates/{sample_1}-{sample_2}/{mode}/closest.{region}.{strand}.bed'
    params:
        out_dir='output/compare_replicates/{sample_1}-{sample_2}/{mode}'
    shell:'''
    mkdir -p {params}
...
rule plot_replicate_closeness:
    conda:
        '../envs/R.yml'
    input:
        closness='output/compare_replicates/{sample_1}-{sample_2}/{mode}/closest.{region}.{strand}.bed',
        samples='samples/GLOE_samples.csv'
    output:
        'output/compare_replicates/{sample_1}-{sample_2}/plots/closest/{mode}/{strand}/boxplot_{region}.png'
    params:
        out_dir = 'output/compare_replicates/{sample_1}-{sample_2}/plots/closest/{mode}/{strand}'
    shell:'''
